...
scripts/headless_cad_writer.py —— 仅调整 import 指向本包，其余保持原样
以便跟随上游 diff 同步。原文件见仓库 scripts/ 目录。
"""
...
from illusion_forge.cad.vendor.cad_core_contracts import (
...
try:
    from illusion_forge.cad.vendor.dxf_preview_scene import dxf_to_preview_scene
except ModuleNotFoundError:
    def dxf_to_preview_scene(*_args: Any, **_kwargs: Any) -> dict[str, Any]:  # type: ignore[misc]
        raise ImportError("dxf_preview_scene 模块不可用")
...
def _run_occt_service(document_path: Path, out_dir: Path, formats: list[str]) -> dict[str, Any]:
    ...
    service = Path(__file__).resolve().parent / "headless_occt_service.py"
...
def _validate_document(document: Any) -> dict[str, Any]:
    ...
    if not isinstance(document, dict):
        raise TypeError("NeutralCadDocument 必须是 JSON object。")
    ...
    if not isinstance(document.get("features", []), list):
        raise TypeError("NeutralCadDocument.features 必须是数组。")
...
def write_dxf(path: Path, document: dict[str, Any]) -> None:
    ...
    doc = ezdxf.new("R2018", setup=True)  # type: ignore[attr-defined]
...
def write_pdf(path: Path, document: dict[str, Any]) -> None:
    ...
    commands = [f"0.97 0.98 0.99 rg 0 0 {page_w:.3f} {page_h:.3f} re f", "0.12 0.16 0.22 RG 0.75 w"]
...
def write_rendered_dxf_format(path: Path, document: dict[str, Any]) -> None:
    ...
    _drawing_mod = importlib.import_module("ezdxf.addons.drawing")
    Frontend = _drawing_mod.Frontend
    RenderContext = _drawing_mod.RenderContext
    ...
    with tempfile.TemporaryDirectory(prefix="cadstudio-drawing-render-") as temp_dir:
        ...
        dxf_document = ezdxf.readfile(source)  # type: ignore[attr-defined]

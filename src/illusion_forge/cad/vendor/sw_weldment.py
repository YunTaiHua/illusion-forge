...
def create_weldment_profile(model: Any, sketch_ref: Any, output_path: str | Path, *, properties: dict[str, Any] | None = None, ) -> dict[str, Any]:
...
def _body_length_key(body: Any) -> int:
    ...
    return round(max(spans) * 1_000_000.0)


def _body_cut_list_key(body: Any) -> tuple[int, int, int]:
    """@brief 生成旋转不变的三轴微米包围盒键，避免同长异型材误合并。"""
    box = tuple(float(item) for item in (get_com_member(body, "GetBodyBox") or ()))
    if len(box) != 6:
        raise RuntimeError("结构构件实体未返回 6 值包围盒")
    spans = sorted((abs(box[3] - box[0]), abs(box[4] - box[1]), abs(box[5] - box[2])))
    return (round(spans[0] * 1_000_000.0), round(spans[1] * 1_000_000.0), round(spans[2] * 1_000_000.0))


def ensure_cut_list(model: Any) -> list[Any]:
...
def _iter_feature_chain(first_feature: Any, next_member: str) -> Any:
    """@brief 遍历顶层或同级子特征链。"""
...
def iter_features_recursive(model: Any) -> Generator[Any, None, None]:
    """@brief 深度遍历焊件文件夹中的 CutListFolder 等子特征。"""
    def walk(feature: Any) -> Generator[Any, None, None]:
        yield feature
        try:
...
def _read_property(manager: Any, name: str) -> dict[str, Any]:
...
def read_property_manager(manager: Any, *, core_only: bool = True) -> dict[str, dict[str, Any]]:
...
def set_cut_list_properties(model: Any, properties: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for feature in iter_features_recursive(model):
        ...
        item: dict[str, Any] = {"name": feature_name, "properties": {}}
...
def update_cut_list(model: Any) -> list[dict[str, Any]]:
...
def weldment_evidence(model: Any, *, create_missing_cut_list: bool = False) -> dict[str, Any]:
    ...
    cut_list_items: list[dict[str, Any]] = []
    try:

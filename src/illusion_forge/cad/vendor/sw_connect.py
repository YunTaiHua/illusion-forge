...
class SolidWorksConnectionError(RuntimeError):
    ...
    def __init__(self, code: Any, stage: Any, message: Any) -> None:
        self.code = code
        self.stage = stage
...
class _LaunchGuard:
    ...
    def __init__(self, name: str="CADStudio.SolidWorks.Launch") -> None:
        self.name = name
        self.handle = None
...
def get_com_member(obj: Any, attr_name: str, *args: Any) -> Any:
    """
...
def normalize_doc_type(doc_type: Any) -> Any:
    """
    规范化文档类型名称。
...
def _expand_path(file_path: Any) -> Any:
    """展开用户目录和环境变量，并返回绝对路径。"""
    return os.path.abspath(os.path.expandvars(os.path.expanduser(file_path)))


def _ensure_parent_dir(file_path: Any) -> Any:
    """确保输出文件的父目录存在。"""
...
def _prog_id_for_version(version: Any) -> Any:
...
def _read_active_document(sw: Any) -> Any:
...
def _wait_until_ready(sw: Any, timeout_seconds: Any) -> Any:
...
def close_owned_solidworks(sw: Any, started_by_cad_studio: Any) -> Any:
...
def connect_solidworks(version: Any=None, wait_seconds: Any=5, visible: bool=True, return_metadata: Any=False) -> Any:
    """
    连接到 SolidWorks 实例。
    ...
    with _LaunchGuard():
        ...
        except (com_error, AttributeError, TypeError, ValueError, OSError, KeyError, IndexError) as attach_error:
            try:
                ...
                raise
            except (com_error, AttributeError, TypeError, ValueError, OSError, KeyError, IndexError) as launch_error:
                close_owned_solidworks(sw, launched_here)
                raise SolidWorksConnectionError(
...
def find_template(sw: Any, doc_type: Any="part") -> Any:
    """
...
def ensure_default_templates(sw: Any) -> Any:
    """
...
def new_document(sw: Any, doc_type: Any="part", template_path: Any=None) -> Any:
    """
    创建新文档。
...
def open_document(sw: Any, file_path: Any, read_only: bool=False, silent: bool=False, raise_on_error: bool=False) -> Any:
    """
    打开已有文档。
...
def save_document(model: Any, file_path: Any=None) -> Any:
    """
    保存文档。

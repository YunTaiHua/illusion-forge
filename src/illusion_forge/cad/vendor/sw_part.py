...
_SKETCH_SELECTION_CACHE: dict[Any, dict[str, Any]] = {}
...
def _select_by_id(extension: Any, entity_name: Any, entity_type: Any, append: bool=False, mark: int=0) -> Any:
    """
...
def _get_plane_name_candidates(plane_name: Any) -> Any:
    """
    获取基准面的候选名称列表
...
def _get_sketch_name_candidates(sketch_name: Any) -> Any:
    """
...
def _safe_com_member(obj: Any, attr_name: str, *args: Any, default: Any = None) -> Any:
...
def _model_cache_key(model: Any) -> Any:
...
def _cache_name_variants(sketch_name: Any) -> Any:
...
def _cache_sketch_ref(model: Any, sketch_ref: Any) -> Any:
...
def _find_cached_sketch_ref(model: Any, sketch_name: Any) -> Any:
    """从当前进程缓存中查找草图对象引用。"""
...
def _capture_sketch_ref(model: Any, sketch_obj: Any=None, fallback: Any="Sketch1", source: Any="") -> Any:
...
def _refresh_sketch_ref(sketch_ref: Any) -> Any:
...
def _select_first_candidate(extension: Any, candidate_names: Any, entity_type: Any, append: bool=False, mark: int=0) -> Any:
    """
    按候选名称列表依次尝试选择实体。
...
def _get_selection_count(model: Any) -> int:
    """
    获取当前选择集中的对象数量。
    ...
    count_member: Any = selection_manager.GetSelectedObjectCount2
    count_value: Any = count_member(-1) if callable(count_member) else count_member
    return int(count_value)


def _select_com_object(obj: Any, append: bool=False, mark: int=0) -> Any:
...
def _select_any_object(objects: Any, append: bool=False, mark: int=0) -> Any:
...
def _select_sketch_ref(model: Any, sketch_ref: Any, mark: int=0) -> Any:
    """
    按稳定优先级选择草图引用。
...
def _select_sketch_feature_by_name(model: Any, sketch_name: Any, mark: int=0) -> Any:
...
def _ensure_sketch_selected(model: Any, sketch_name: Any) -> Any:
    """
    确保拉伸/切除前已有可用的草图选择。
...
def start_sketch(model: Any, plane_name: Any="Front Plane") -> Any:
    """
    在指定基准面上开始草图
...
@contextmanager
def sketch(model: Any, plane_name: Any="Front Plane") -> Any:
    """
    草图上下文管理器。
...
def current_sketch_name(model: Any, fallback: Any="Sketch1") -> Any:
    """
    获取当前草图名称。
...
def sketch_line(model: Any, x1: Any, y1: Any, x2: Any, y2: Any) -> Any:
    """画直线（单位: 米）"""
    return model.SketchManager.CreateLine(x1, y1, 0, x2, y2, 0)


def sketch_rectangle(model: Any, cx: Any, cy: Any, w: Any, h: Any) -> Any:
    """以中心点画矩形（单位: 米）"""
...
def sketch_corner_rectangle(model: Any, x1: Any, y1: Any, x2: Any, y2: Any) -> Any:
    """以对角线画矩形（单位: 米）"""
    return model.SketchManager.CreateCornerRectangle(x1, y1, 0, x2, y2, 0)


def sketch_circle(model: Any, cx: Any, cy: Any, radius: float) -> Any:
    """画圆（单位: 米）"""
    return model.SketchManager.CreateCircleByRadius(cx, cy, 0, radius)


def sketch_arc(model: Any, cx: Any, cy: Any, x1: Any, y1: Any, x2: Any, y2: Any, direction: Any=1) -> Any:
    """
    画圆弧
...
def sketch_polygon(model: Any, cx: Any, cy: Any, radius: float, sides: Any=6) -> Any:
    """画正多边形（内切圆方式）"""
    return model.SketchManager.CreatePolygon(cx, cy, 0, cx + radius, cy, 0, sides, True)


def sketch_slot(model: Any, x1: Any, y1: Any, x2: Any, y2: Any, radius: float) -> Any:
    """画槽口"""
...
def sketch_spline(model: Any, points: Any) -> Any:
    """
    画样条曲线
...
def auto_dimension_sketch(model: Any, sketch: Any=None, sketch_segments: Any=None) -> Any:
    ...
    if active is None:
        blocked: dict[str, Any] = {"status": "blocked", "status_code": None,
                                   "error_code": "SKETCH_NOT_ACTIVE", "selected": 0}
        return blocked

    def segments() -> tuple[Any, ...]:
...
def add_dimension(model: Any, x: float, y: float) -> Any:
    """在指定位置添加尺寸标注"""
    return model.AddDimension2(x, y, 0)


def add_sketch_relation(model: Any, relation_type: Any) -> Any:
    """
    添加草图几何关系
...
def extrude_boss(model: Any, sketch_name: Any, depth: float, direction: Any=True, merge: Any=True) -> Any:
    """
    凸台拉伸
...
def extrude_cut(model: Any, sketch_name: Any, depth: float, direction: Any=True, flip: Any=False) -> Any:
    """
    切除拉伸
...
def extrude_midplane(model: Any, sketch_name: Any, total_depth: Any) -> Any:
    """
    中面拉伸（两侧对称拉伸）
...
def revolve_boss(model: Any, sketch_name: Any, angle_rad: Any, axis_sketch_name: Any=None) -> Any:
    """
    旋转凸台
...
def fillet(model: Any, radius: float, edges: Any=None) -> Any:
    """
    倒圆角
...
def chamfer(model: Any, distance: float, angle_deg: Any=45) -> Any:
    """
    倒角
...
def linear_pattern(model: Any, feature_name: str, d1_x: float, d1_y: float,
                   d1_z: float, d1_spacing: float, d1_count: int,
                   d2_x: float = 0, d2_y: float = 0, d2_z: float = 0,
                   d2_spacing: float = 0, d2_count: int = 1) -> Any:
    """
    线性阵列
...
def circular_pattern(model: Any, feature_name: Any, axis_name: Any, angle_rad: Any, count: int, equal_spacing: bool=True) -> Any:
    """
    圆形阵列
...
def shell(model: Any, thickness: float, faces_to_remove: Any=None) -> Any:
    """
    抽壳
...
def mirror_feature(model: Any, feature_name: Any, mirror_plane_name: Any) -> Any:
    """
    镜像特征
...
def hole_wizard(model: Any, hole_type: Any, standard: Any, fastener_type: Any, size: Any, depth: float, x: float, y: float, z: float) -> Any:
    """
    异型孔向导（简化版）
...
def rib(model: Any, sketch_name: Any, thickness: float, direction: Any=True) -> Any:
    """
    筋特征

...
scripts/headless_occt_service.py —— 仅移除原仓库路径注入 hack，其余原样。
"""
...
def build_shape(document: dict[str, Any]) -> Any:
    """@brief 从 NeutralCadDocument 构造 OCCT TopoDS_Shape。"""
    from OCP.BRepAlgoAPI import BRepAlgoAPI_Cut, BRepAlgoAPI_Fuse  # type: ignore[import-not-found]
    from OCP.BRepPrimAPI import (  # type: ignore[import-not-found]
        BRepPrimAPI_MakeBox,
        BRepPrimAPI_MakeCylinder,
    )
    from OCP.gp import gp_Ax2, gp_Dir, gp_Pnt  # type: ignore[import-not-found]
...
def collect_evidence(shape: Any) -> dict[str, Any]:
    """@brief 回读有效性、体积、包围盒和拓扑数量。"""
    from OCP.Bnd import Bnd_Box  # type: ignore[import-not-found]
    from OCP.BRepBndLib import BRepBndLib  # type: ignore[import-not-found]
    ...
    from OCP.TopExp import TopExp_Explorer  # type: ignore[import-not-found]
...
def _mesh_shape(shape: Any, linear_deflection: float = 0.2, angular_deflection: float = 0.35) -> None:
...
def export_occt(
    ...
    *,
    ...
    from OCP.BRepTools import BRepTools  # type: ignore[import-not-found]
    ...
    from OCP.STEPControl import (  # type: ignore[import-not-found]
        STEPControl_AsIs,
        STEPControl_Writer,
    )
    from OCP.StlAPI import StlAPI_Writer  # type: ignore[import-not-found]

    from illusion_forge.cad.vendor.cad_core_contracts import sha256_file

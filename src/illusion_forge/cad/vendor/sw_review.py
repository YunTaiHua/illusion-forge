...
_CONFIDENCE_RANK = {"unavailable": 0, "low": 1, "medium": 2, "high": 3}


def _confidence_rank(value: object) -> int:
    """@brief 置信度字符串 → 排序序号（未知值按 unavailable）。"""
    return _CONFIDENCE_RANK.get(str(value), 0)


def _drawing_boxes_overlap(first: Any, second: Any, padding_m: float=0.001) -> bool:
...
def review_drawing_layout(structure: Any, *, padding_m: float=0.001, preview_evidence: Any=None) -> dict[str, Any]:
    ...
    def add_finding(code: Any, severity: Any, first_kind: Any, first_name: Any, second_kind: Any, second_name: Any, *, evidence_source: Any="native", confidence: Any="high") -> Any:
    ...
    def same_sheet(first: Any, second: Any) -> Any:
    ...
    def is_owner_view_label(note: Any, view: Any) -> Any:
...
def _expand_path(path: Any) -> Any:
    """展开输出路径。"""
    return Path(os.path.expandvars(str(path))).expanduser().resolve()


def _file_info(path: Any) -> Any:
    """返回文件存在性和大小信息。"""
...
def _read_bmp_size(path: Any) -> Any:
    """
    读取 BMP 宽高。
...
def inspect_pdf_text_layout(path: Any, *, maximum_spans: Any=20000, minimum_overlap_area_pt2: Any=0.5) -> Any:
...
def inspect_bmp_preview(path: Any, sample_limit: Any=200000) -> Any:
    """
    对 BMP 预览图做轻量检查。
    ...
    try:
        ...
        dark = 0
        total = 0
        if pixel_offset > 0 and row_stride > 0 and channels and width:
            # BMP 高度为正时像素自底向上；负高度表示自顶向下。
            signed_height: int = int.from_bytes(data[22:26], "little", signed=True)
...
def zoom_to_fit(model: Any) -> Any:
    """缩放到适合窗口并刷新图形。"""
    get_com_member(model, "ViewZoomtofit2")
    get_com_member(model, "GraphicsRedraw2")


def clear_selection_for_preview(model: Any) -> Any:
...
def activate_model_for_preview(model: Any) -> Any:
...
def set_standard_view(model: Any, view_name: Any="isometric") -> Any:
    """
    设置标准视图方向。
...
def save_preview(model: Any, output_path: Any, view_name: Any="isometric", width: float=1600, height: float=1000) -> Any:
    """
    导出当前模型预览图。
...
def save_review_previews(model: Any, output_dir: Any, basename: Any="review", views: Any=None) -> Any:
    """
    导出多视角预览图。
...
def collect_model_summary(model: Any) -> Any:
    """
    收集基础模型摘要。
...
def _axis_distance_mm(point_mm: Any, origin_mm: Any, axis: Any) -> Any:
...
def group_coaxial_hole_segments(segments: Any, position_tolerance_mm: Any=0.05, axis_tolerance: Any=1e-5) -> Any:
    """
...
def validate_hole_positions(measurements: Any, expected_holes: Any, position_tolerance_mm: Any=0.1, diameter_tolerance_mm: Any=0.05) -> Any:
    """
    ...
    for index, expected in enumerate(expected_holes):
        ...
        best_diameter = best[3] if best else None
        best_position = best[4] if best else None
        passed = bool(
            best is not None
            and best_diameter is not None
            and best_position is not None
            and best_diameter <= diameter_tolerance_mm
            and best_position <= position_tolerance_mm
        )
        if passed and best:
            used.add(best[1])
        checks.append({
...
def collect_geometry_measurements(model: Any) -> Any:
    """
    ...
    try:
        ...
        if len(box) >= 6:
            ...
            measurements.setdefault("errors", []).append("GetPartBox(True) 未返回 6 个坐标值")
    except (com_error, AttributeError, TypeError, ValueError, OSError, KeyError, IndexError) as exc:
        measurements.setdefault("errors", []).append(f"包围盒读取失败: {exc}")
            ...
            for face_index, face in enumerate(get_com_member(body, "GetFaces") or []):
                try:
                    ...
                    if internal:
                        ...
                        if int(cylinder.get("edge_count") or 0) <= 2:  # type: ignore[call-overload]
...
def build_review_report(model: Any, output_dir: Any, basename: Any="review", views: Any=None, expected_outputs: Any=None) -> Any:
    """
    生成结构化审查报告数据。
...
def evaluate_review_report(report: Any) -> Any:
    """
    对结构化审查报告做规则评分。
    ...
    def add_issue(code: Any, severity: Any, message: Any, recommendation: Any, penalty: Any) -> Any:
    ...
    for preview in previews:
        ...
        if preview.get("width") and preview.get("height") and (
                preview["width"] < 640 or preview["height"] < 480):
                add_issue(
...
def write_review_report(report: Any, output_path: Any) -> Any:
    """
    写入 JSON 审查报告。
...
def write_markdown_summary(report: Any, output_path: Any) -> Any:
    """
    写入 Markdown 审查摘要。
...
def run_review(model: Any, output_dir: Any, basename: Any="review", views: Any=None, expected_outputs: Any=None) -> Any:
    """
...
def main() -> Any:
    """命令行入口。"""
    args = _parse_args()
    sw_model = connect_solidworks(version=args.version)
    sw, model = sw_model[0], sw_model[1]
    if args.file:

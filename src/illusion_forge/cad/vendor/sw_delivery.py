...
def _file_signature(path: Path) -> Any:
    if not path.is_file():
        return None
    stat = path.stat()
...
def _property_value(model: Any, configuration_name: str, names: tuple[str, ...]) -> str:
    if model is None:
        return ""
    for scope in (configuration_name, ""):
        for name in names:
            try:
                ...
                continue
            if value["exists"]:
                return str(value["resolved"] or value["raw"])
    return ""


def collect_assembly_bom(model: Any, *, include_excluded: bool = False) -> list[dict[str, Any]]:
...
def export_assembly_bom_csv(model: Any, output_path: str, *, include_excluded: bool = False, overwrite: bool = False, ) -> dict[str, Any]:
...
def build_bom_traceability(bom_rows: Any, *, model_path: Any="", drawing_path: Any="", review_path: Any="") -> dict[str, Any]:
...
def _status_codes(value: str) -> list[int]:
    if value is None:
        return []
...
def _collect_new_outputs(target: Path, existing_files: dict[str, tuple[int, int] | None]) -> list[dict[str, Any]]:
    ...
    for path in sorted(item for item in target.rglob("*") if item.is_file()):
        ...
        produced = after is not None and after != before
        if produced and after is not None:
            outputs.append({
                "path": str(path),
                "size_bytes": after[0],
                "sha256": _sha256(path),
...
def _document_dependency_paths(model: Any, source_path: str) -> list[str]:
...
def _optional_com_member(obj: Any, name: str, *args: Any, default: Any = None) -> Any:
...
def _toolbox_state(component: Any, referenced_model: Any, path: str) -> tuple[str, str]:
...
def _suppression_state(component: Any) -> tuple[str, int | None]:
    ...
    try:
        state = int(raw_state) if raw_state is not None else -1
    except (TypeError, ValueError):
...
def _collect_component_audit_records(model: Any, source_path: str) -> list[dict[str, Any]]:
...
def build_pack_and_go_audit_matrix(
    ...
    *,
    ...
    tracked_names = {str(row.get("file_name") or "").casefold() for row in rows}
    for output in outputs:
...
def _comtypes_document_names(package: Any, document_count: int) -> list[str]:
...
def _load_sldworks_typelib_module() -> Any:
...
def _model_doc_extension(model: Any) -> Any:
    ...
    extension = typed_model.Extension if typed_model is not None else None
    if extension is None:
        raise RuntimeError("SolidWorks 未返回 IModelDocExtension")
    return extension


def _coerce_dispatch(value: Any) -> Any:
...
def _get_pack_and_go(extension: Any) -> Any:
...
def _pywin32_pack_and_go(extension: Any, target: Path, existing_files: dict[str, tuple[int, int] | None] | None, *, include_drawings: bool, include_simulation_results: bool, include_toolbox_components: bool, include_suppressed: bool, flatten: bool) -> dict[str, Any]:
    ...
    outputs = _collect_new_outputs(target, existing_files or {})
    return {
        "backend": "pywin32",
...
def _extract_comtypes_model(value: Any) -> Any:
    ...
    SldWorks = importlib.import_module("comtypes.gen.SldWorks")
...
def _comtypes_active_model(sw: Any, source_path: str) -> Any:
...
def _connect_comtypes_solidworks(client: Any, progids: list[str]) -> Any:
...
def pack_and_go(model: Any, output_dir: Any, *, include_drawings: bool = True, include_simulation_results: bool = False, include_toolbox_components: bool = True, include_suppressed: bool = False, flatten: bool = False, overwrite: bool = False, fallback_policy: str = "stage_dependencies", ) -> dict[str, Any]:
    ...
    try:
        ...
        except (com_error, AttributeError, TypeError, ValueError, OSError, KeyError, IndexError) as comtypes_exc:

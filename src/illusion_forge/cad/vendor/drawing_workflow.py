...
Vendored from wzyn20051216/solidworks-automation-skill (MIT License),
subskills/solidworks-engineering-drawing/scripts/drawing_workflow.py ——
仅将 import 指向本包内 vendored 模块，其余保持原样。
"""
import logging
import math
import re
...
from illusion_forge.cad.vendor.sw_connect import create_empty_dispatch_variant, get_com_member
from illusion_forge.cad.vendor.sw_preflight import import_com_dependencies

pythoncom, _win32com, VARIANT = import_com_dependencies()


PAPER_SIZES: dict[str, dict[str, Any]] = {
...
def _safe_member(obj: Any, name: str, *args: Any, default: Any = None) -> Any:
...
def _normalise_box(value: Any) -> Any:
...
def _annotation_box(owner: Any) -> Any:
...
def _view_display_dimensions(view: Any) -> Any:
...
def auto_arrange_drawing_dimensions(drawing_model: Any, *, spacing_m: Any=0.01, mode: Any="auto_arrange") -> dict[str, Any]:
    ...
    for sheet_name in sheets or [""]:
        ...
        for view in _as_sequence(_safe_member(sheet, "GetViews", default=[])):
            ...
            if attempted:
                ...
                except (com_error, AttributeError, TypeError, ValueError, OSError, KeyError, IndexError) as exc:
                    error = str(exc)
            results.append({
...
def _finite_positive(value: Any, default: Any) -> Any:
...
def _dimension_text_evidence(display_dimension: Any) -> dict[str, Any]:
...
def estimate_dimension_text_box(display_dimension: Any, *, padding_m: float | None = None) -> dict[str, Any]:
...
def select_drawing_template(candidates: Any, *, paper_size: Any="A3", require_gbt: bool=True) -> dict[str, Any]:
    ...
    selected = max(eligible, key=lambda item: (item["score"], str(item["path"]).casefold()), default=None)
    return {
...
def plan_standard_view_layout(model_size_m: Any, *, paper_size: Any="A3", projection: Any="third_angle", requested_scale: Any=None, margin_m: Any=0.012, gap_m: Any=0.018, title_block_width_m: Any=0.180, title_block_height_m: Any=0.055, ) -> dict[str, Any]:
    ...
    def view_record(name: str, left: Any, bottom: Any, view_width: Any, view_height: Any) -> dict[str, Any]:
...
def _scale_ratio(scale: float) -> Any:
    """@brief 将浮点比例转换为 SolidWorks 可接受的整数比。"""
    ratio = Fraction(float(scale)).limit_denominator(1000)
    return ratio.numerator, ratio.denominator


def _apply_view_layout(view: Any, item: Any, numerator: Any, denominator: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "center": center,
        "actual_center": list(actual_position or ()),
        "position_verified": True,
        "scale_ratio": [int(numerator), int(denominator)],
    }


def _normalise_orientation(value: Any) -> Any:
...
def _view_position(view: Any) -> Any:
    """@brief 读取工程图视图中心位置。"""
...
def _set_view_center(view: Any, center: Any) -> list[float]:
...
def _refine_standard_view_spacing(view_objects: Any, view_records: Any, projection: Any, gap_m: Any=0.018) -> dict[str, Any]:
    ...
    def move_if_needed(axis: Any, orientation: str, target_value: Any) -> Any:
    ...
    right = boxes["right"]
    top = boxes["top"]
    if front is None or right is None or top is None:
        # 任一视图缺少有效包围盒时无法做布局精细化——如实上报，
        # 不得伪装 pass 让下游自动化跳过人工复核
        return {
            "status": "review_required",
            "error_code": "DRAWING_VIEW_BOX_INCOMPLETE",
            "stage": "layout",
            "message": "标准视图缺少有效包围盒，布局精细化已跳过，需人工确认视图位置。",
            "views": records,
        }
...
def _collect_drawing_views(drawing_model: Any) -> Any:
...
def _map_native_standard_views(views: Any) -> Any:
    ...
    for view in views:
        ...
        if orientation and orientation not in by_orientation and view is not None:
    ...
    positioned = [(v, pos, base) for (v, pos, base) in positioned if pos is not None]
    if len(positioned) >= 3:
...
def _map_standard_views_from_sheet_views(sheet_views: Any) -> Any:
...
def create_adaptive_standard_views(drawing_model: Any, part_path: Any, layout: Any) -> dict[str, Any]:
...
def create_standard_views(drawing_model: Any, part_path: Any) -> Any:
    """
    创建标准三视图（第三角投影法）
...
def create_standard_views_with_projection(drawing_model: Any, part_path: Any, projection: Any="first_angle") -> Any:
...
def add_view(drawing_model: Any, part_path: Any, view_name: Any, x: float, y: float, scale: float | None = None) -> Any:
    """
    添加单个视图
...
def add_section_view(drawing_model: Any, x: float, y: float) -> Any:
    """在当前选择的剖切线位置创建剖视图"""
    return drawing_model.CreateSectionViewAt5(x, y, 0, "", 0, None, 0)


def add_detail_view(drawing_model: Any, x: float, y: float, scale: float=2.0) -> Any:
    """创建局部放大视图"""
    return drawing_model.CreateDetailViewAt4(x, y, 0, 0, scale, 0, "")


def insert_dimensions(drawing_model: Any, view: Any=None) -> Any:
    """
    自动标注尺寸（模型项目）
...
def _note_annotation(note: Any) -> Any:
...
def _set_annotation_position(annotation: Any, x: float, y: float, z: float=0.0) -> Any:
...
def _annotation_position(annotation: Any) -> Any:
...
def _note_box(note: Any, annotation: Any) -> Any:
...
def _note_evidence(note: Any) -> Any:
...
def add_note(drawing_model: Any, x: float, y: float, text: Any) -> Any:
    """@brief 创建并验证工程图文字注释。
...
def insert_bom_table(drawing_model: Any, template_path: Any, x: float, y: float, bom_type: Any=1, config_name: Any="") -> Any:
    """
    插入 BOM 表
...
def set_sheet_format(drawing_model: Any, format_path: Any) -> Any:
    """
    设置图纸格式（图框）
...
def add_sheet(drawing_model: Any, paper_size: int=7, template_path: Any="", first_angle: bool=True) -> Any:
    """
    添加新图纸
...
def add_a3_sheet(drawing_model: Any, template_candidates: Any, *, require_gbt: bool=True, projection: Any="first_angle") -> dict[str, Any]:
...
def setup_current_sheet_as_a3(drawing_model: Any, template_candidates: Any, *, require_gbt: bool=True, projection: Any="first_angle") -> dict[str, Any]:
...
def get_all_views(drawing_model: Any) -> Any:
    """获取当前图纸上的所有视图"""
...
def _paper_size_from_template(template_path: Any) -> Any:
...
def _table_cell_text(table: Any, row: int, column: int) -> Any:
...
def _table_evidence(table: Any, sheet_name: Any) -> Any:
    """@brief 回读表格类型、数据行、单元格及 BOM 配置证据。"""
    table_type = _safe_member(table, "Type", default=None)
    try:
        normalized_type = int(table_type) if table_type is not None else 0
    except (TypeError, ValueError):
        normalized_type = 0
    try:
    ...
    return {
        ...
        "kind": TABLE_ANNOTATION_KINDS.get(int(normalized_type or 0), "unknown"),
...
def _annotation_text_parts(annotation_owner: Any) -> Any:
...
def _professional_annotation_record(owner: Any, kind: Any, view_record: Any) -> Any:
...
def _view_center_marks(view: Any) -> Any:
...
def _collect_view_professional_annotations(view: Any, view_record: Any) -> Any:
...
def inspect_drawing_structure(drawing_model: Any, *, paper_size_hint: Any=None, title_block_box: Any=None) -> dict[str, Any]:
    ...
    notes: list[Any] = []
    tables: list[Any] = []
    professional_annotations: dict[str, Any] = {
        "center_marks": [],
        "center_lines": [],
        "hole_callouts": [],
        "datum_tags": [],
        ...
        for view in sheet_views:
            ...
            for dimension in view_dimensions:
                ...
                dimension_record = {
                    ...
                    "box": native_box or (estimated_box_evidence or {}).get("box"),
                    "box_source": "native" if native_box else (estimated_box_evidence or {}).get("source"),
                    "box_confidence": "high" if native_box else (estimated_box_evidence or {}).get("confidence"),
                    "box_evidence": {
                        "box": native_box,
                        "source": "native",
                        "confidence": "high",
            ...
            for note in _as_sequence(_safe_member(view, "GetNotes", default=[])):
                ...
                note_text = str((note_evidence or {}).get("text") or "").strip()
            ...
            for table in _as_sequence(_safe_member(view, "GetTableAnnotations", default=[]) or []):
                tables.append(_table_evidence(table, sheet_name))
    current_sheet = _safe_member(drawing_model, "GetCurrentSheet")
    template = str(_safe_member(current_sheet, "GetTemplateName", default="") or "")
    paper_size = str(paper_size_hint or _paper_size_from_template(template) or "").upper() or None
    spec = PAPER_SIZES.get(paper_size) if paper_size else None
...
def auto_insert_center_marks(drawing_model: Any, requirements: Any) -> dict[str, Any]:
    ...
    sheet = _safe_member(drawing_model, "GetCurrentSheet")
    sheet_views = _as_sequence(_safe_member(sheet, "GetViews", default=[]) or [])
    ...
    for requirement in requested:
        ...
        view = mapped_views.get(orientation) if orientation else None
...
def export_sheet_to_pdf(model: Any, output_path: Any, sheet_names: Any=None, sw_app: Any=None) -> Any:
    """
    将工程图导出为 PDF
    ...
    if sw is None:
        for prog_id in ("SldWorks.Application.32", "SldWorks.Application"):
            try:
                ...
                logging.getLogger(__name__).debug('vendor 边界失败，跳过', exc_info=True)
                continue
    if sw is None:
        return False
    try:
...
def setup_current_sheet(drawing_model: Any, template_candidates: Any, *, paper_size: Any="A3", projection: Any="first_angle", require_gbt: bool=True) -> dict[str, Any]:
...
def validate_generic_drawing_generation(spec: Any) -> dict[str, Any]:
...
def generate_drawing_from_spec(drawing_model: Any, spec: Any, source_model_path: Any, template_candidates: Any=None) -> dict[str, Any]:
